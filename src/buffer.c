...
static void _readFromSocket(struct event * _ev, int signal, void *_args){
    if( _ev == (struct event *)NULL \
        || _args == (void *)NULL ){
        LOG_ERROR("Read from socket error, empty point of event");
        return ;
    }
    if( signal & EV_ERR ){
        goto err;
    ...
    while(1){
        ...
        LOG_DEBUG("readFromSocket be called and nRecv = %d", nRecv);
        if( nRecv < 0 ){
            if(errno == EAGAIN || errno == EINTR){
                break;
            }else{
                LOG_ERROR("Error in readFromSocket:%s", strerror(errno));
                goto err;
            }
        }else if(nRecv == 0){
    ...
    if( _ev->evCb == (evCallback *)NULL ){
        LOG_ERROR("Read Callback not define");
        goto err;
    }
    ...
    while(1){
        ...
        if( nSend < 0 ){
            if(errno == EAGAIN || errno == EINTR){
                ...
                LOG_ERROR("error in write to socket:%s", strerror(errno));
                goto err;
            }
        }
        break;
...
int bufferWrite(struct buffer * _B, char *buf, int n){
    ...
    if( _B->nWBufLeave < n ){
        LOG_ERROR("Buffer Error, No enough memory");
        goto err;
    }

    int nSend = send(_B->ev.fd, buf, n, MSG_NOSIGNAL | MSG_DONTWAIT);
    LOG_DEBUG("Send to Client %d Byte", nSend);
    if( nSend < 0 ){
        if(errno == EAGAIN || errno == EINTR){
...
struct buffer * createBuffer(struct reactor * _R, int fd){
    ...
    {
        LOG_ERROR("In Buffer create, error of eventSelfInit");
        goto err;
    }
